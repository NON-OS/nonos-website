...
var Wallet = {
    ...
    openWalletConnect: function() {
        if (window.appKit && typeof window.appKit.open === 'function') {
            ...
            UI.showToast('Loading WalletConnect...', 'info');
            var attempts = 0;
            var checkInterval = setInterval(function() {
                ...
                if (window.appKit && typeof window.appKit.open === 'function') {
                    ...
                    clearInterval(checkInterval);
                    UI.showToast('WalletConnect unavailable - use mobile deep links below', 'error');
                }
            }, 250);
        }
    },
...
var NFT = {
    loadUserNFTs: async function() {
        ...
        try {
            var url = 'https://api.reservoir.tools/users/' + Wallet.userAddress + '/tokens/v10?collection=' + CONFIG.ZEROSTATE_PASS_ADDRESS + '&limit=10';
            ...
            if (!data.tokens || data.tokens.length === 0) {
                ...
                gallery.innerHTML = '<div class="nft-empty">' +
                    ...
                    '<p>No Zero State Pass NFTs found</p>' +
                    '</div>';
                return;
            }
            ...
            for (var i = 0; i < count; i++) {
                ...
                var imageUrl = token.image || token.imageSmall || ('https://api.reservoir.tools/redirect/tokens/' + CONFIG.ZEROSTATE_PASS_ADDRESS + ':' + tid + '/image/v1');

                html += '<div class="nft-card">' +
                    '<div class="nft-image-container">' +
                    '<img src="' + imageUrl + '" alt="' + name + '" class="nft-image" loading="lazy" onerror="this.style.display=\'none\'">' +
                    '</div>' +
                    '<div class="nft-info">' +
                    '<span class="nft-name">' + name + '</span>' +
                    '<span class="nft-boost">+' + UI.getBoostForNftPosition(i + 1) + '% boost</span>' +
                    '</div>' +
                    '</div>';
            }
