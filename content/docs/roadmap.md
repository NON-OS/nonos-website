...
|-------|------------|-------|------|
| Block devices | AHCI, NVMe, VirtIO | Basic | Production |
| Partition tables | GPT | Read-only | Read/write |
| Filesystems | RAM-only | Functional | ext4 + CryptoFS |
| Encryption | None | - | Full-disk mandatory |
| Integrity | None | - | dm-verity equivalent |

### Network Stack

| Layer | Technology | Alpha | Beta |
|-------|------------|-------|------|
| Link | e1000, RTL8139, VirtIO-net | Basic | Production |
| IP | IPv4, IPv6 | Functional | Hardened |
| Transport | TCP, UDP | Functional | RFC-compliant |
| Privacy | None | - | Onion routing default |
| DNS | Cleartext | Basic | DoH/DoT + Onion |

### Hardware Support

| Category | Alpha | Beta |
|----------|-------|------|
| CPU | x86_64 only | x86_64 (ARM64 planned) |
| Boot | UEFI only | UEFI + legacy BIOS |
| Storage | AHCI, NVMe | + USB mass storage |
| Network | Basic NICs | + WiFi (select chipsets) |
| Input | PS/2 keyboard | + USB HID |
| Graphics | VGA text, framebuffer | + basic GPU |
| Security | None | TPM 2.0 integration |
